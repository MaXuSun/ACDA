...
from model.attention import DAAttention as SelfA
...
class BaseSolver:
    def __init__(self, net, dataloader,resume=None,**kwargs):
        ...
        self.net = net
        atten_sizes = [torch.Size([-1, 512, 28, 28]),
                       torch.Size([-1, 1024, 14, 14]),
                       torch.Size([-1, 2048, 7, 7])]
        self.new_size = torch.Size([-1,32,7,7])
        self.att_feat_dim = 32*7*7
        self.attention = to_cuda(SelfA(atten_sizes,new_size=self.new_size))
    ...
    def init_data(self, dataloader):
        ...
        if 'test' in dataloader:
            ...
            self.test_data['s_loader'] = dataloader['s_test']
        
    def build_optimizer(self):
        opt = self.opt
        param_groups = solver_utils.set_param_groups(self.net, 
		dict({'FC': opt.TRAIN.LR_MULT}))
        param_groups_att = solver_utils.set_param_groups_att(self.attention)
        ...
        if opt.TRAIN.OPTIMIZER == "Adam":
            ...
            # TODO attention optimizer
            self.optimizer_att = optim.Adam(param_groups_att,lr=self.base_lr,betas=[opt.ADAM.BETA1, opt.ADAM.BETA2],
			weight_decay=0.0005)
			...
			weight_decay=0.0005)
            # TODO　attention optimizer
            self.optimizer_att = optim.SGD(param_groups_att,lr=self.base_lr,
    ...
    def update_lr(self):
        ...
        if self.opt.TRAIN.LR_SCHEDULE == 'exp':
            ...
            # TODO attention optimizer lr
            solver_utils.adjust_learning_rate_exp(self.base_lr,
                                                  self.optimizer_att,iters,
                                                  decay_rate=self.opt.EXP.LR_DECAY_RATE,
                                                  decay_step=self.opt.EXP.LR_DECAY_STEP)
            ...
            # TODO attention optimizer lr
            solver_utils.adjust_learning_rate_inv(self.base_lr, self.optimizer_att,
                                                  iters, self.opt.INV.ALPHA, self.opt.INV.BETA)
    ...
    def s_test(self):
        ...
        gts = []
        feats = []
        for sample in iter(self.test_data['s_loader']):
            data, gt = to_cuda(sample['Img']), to_cuda(sample['Label'])
            out = self.net(data)
            logits = out['logits']
            feats += [out['feat']]
