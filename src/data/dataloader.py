...
from .dataset import CategoricalSTDataset
...
class CustomDatasetDataLoader(object):
    ...
    def __init__(self, dataset_type, train, batch_size, 
        ...
        self.train = train
        self.dataset = getattr(dataset, dataset_type)()
...
class ClassAwareDataLoader(object):
    ...
    def __len__(self):
        ...
        for c in self.class_set:
            c_len = max([len(self.dataset.data_paths[d][cid]) // self.dataset.batch_sizes[d][cid] for d in ['source', 'target']])
            dataset_len += c_len
            cid += 1
