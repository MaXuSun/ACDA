...
class CDD(object):
    ...
    def forward(self, source, target, nums_S, nums_T):
        ...
        for i in range(self.num_layers):
            ...
            cur_source = source[i]              # N * f_n_dim
            cur_target = target[i]              # N * f_n_dim
    ...
    def kernel_layer_aggregation(self, dist_layers, gamma_layers):
        ...
        for i in range(num_layers):
            ...
            if len(kernel_dist.keys()) == 0:  
                kernel_dist = {key: self.compute_kernel_dist(dist[key], gamma, cur_kernel_num, cur_kernel_mul) for key in ['ss', 'tt', 'st']}
                continue
            if not self.joint:
                kernel_dist = {key: kernel_dist[key] + self.compute_kernel_dist(dist[key], gamma, cur_kernel_num, cur_kernel_mul) for key in ['ss', 'tt', 'st']}
            else:            
                kernel_dist = {key: kernel_dist[key] * self.compute_kernel_dist(dist[key], gamma, cur_kernel_num, cur_kernel_mul) for key in ['ss', 'tt', 'st']}
